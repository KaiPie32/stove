...
def get_packages():
    """Fetch the apps list from GitHub"""
    response = requests.get(PACKAGES_URL)
    response.raise_for_status()
    return response.json()

def download_and_extract(url, app_name):
    """Download a zip from a URL and extract it into StoveApps"""
...
def cook(app_name):
    """Install app by name from packages.json"""
    data = get_packages()
...
🍳 Stove Package Manager Commands:
  ...
  stove cook --app <name>       - Install an app from URL
  stove cook --local <zip>      - Install an app from local zip
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        list_apps()
    elif sys.argv[1] == "cook" and len(sys.argv) > 2:
        if sys.argv[2] == "--app" and len(sys.argv) > 3:
            cook(sys.argv[3])
        elif sys.argv[2] == "--local" and len(sys.argv) > 3:
            cook_local(sys.argv[3])
        else:
            print("❌ Invalid cook command. Run 'stove help'.")
    elif sys.argv[1] == "update":
        update()
    elif sys.argv[1] == "help":
        help()
    else:
        print("❌ Invalid command. Run 'stove help'.")
